...
# ── Memory safety ──────────────────────────────────────────────────────────────
# Force Python allocator to release memory back to the OS after every free,
# preventing the silent RSS growth that OOMs long-running tile loops.
try:
    import ctypes
...
# How often (in tiles) to force a full GC + CUDA cache flush.
_GC_EVERY_N_TILES = int(os.environ.get("PIPELINE_GC_INTERVAL", "50"))
...
def write_overlay_raster(source_path: Path, overlay_path: Path, mask_array: np.memmap) -> None:
    # block_height = 256 rows  →  peak RAM per block ≈ width × 256 × 3 bytes × 2 (read+write)
    # For a 14929-wide uint16 5-band raster: ~11 MB/block  (was ~90 MB at 1024 rows)
    block_height = 256
    with rasterio.open(source_path) as src:
        profile = src.profile.copy()
...
def _maybe_gc(tile_index: int) -> None:
    """Force GC + CUDA cache flush every N tiles to cap RAM growth."""
    if tile_index % _GC_EVERY_N_TILES == 0:
        gc.collect()
    ...
    with rasterio.open(source) as src:
        ...
        for index, job in enumerate(jobs, start=1):
            ...
            if boxes is None or len(boxes) == 0:
                ...
                print(f"[{index}/{len(jobs)}] tile {job.tile_id}: no detections", flush=True)
                del rgb_tile, detections, boxes
                _maybe_gc(index)
                continue
            ...
            if not kept_indexes:
                ...
                print(f"[{index}/{len(jobs)}] tile {job.tile_id}: overlap-only detections skipped", flush=True)
                del rgb_tile, detections, boxes, raw_boxes
                _maybe_gc(index)
                continue
            ...
            # ── explicit cleanup every tile ────────────────────────────────────
            del rgb_tile, detections, boxes, raw_boxes, prompt_boxes, sam_result
            _maybe_gc(index)
