...
def _estimate_point_spacing(coordinates: np.ndarray) -> tuple[float, float]:
    """Estimate local plant spacing from nearest-neighbor distances."""
...
def _build_cluster_models(coordinates: np.ndarray, labels: np.ndarray) -> dict[int, dict]:
    """Summarize each non-noise cluster in a local PCA coordinate frame."""
...
def _reassign_noise_points(coordinates: np.ndarray, labels: np.ndarray) -> np.ndarray:
    """Rescue edge points that clearly belong to an existing sector."""
...
def _relabel_sectors_by_position(labels: np.ndarray, coordinates: np.ndarray) -> np.ndarray:
    """Renumber sectors in a stable top-to-bottom, left-to-right order."""
...
def cluster_plants_hdbscan(centroids: list[dict], min_cluster_size: int = 10) -> list[dict]:
    """Assign sector IDs using HDBSCAN plus geometry-aware post-processing."""
    if not centroids:
        return []
...
def _estimate_axis_tolerance(axis_values: np.ndarray) -> float:
    """Estimate grouping tolerance from the median non-zero nearest-neighbor distance."""
...
def assign_row_col_within_sector(centroids: list[dict]) -> list[dict]:
    """Assign row and column indices inside each sector using sector-local orientation."""
    if not centroids:
        return []
    ...
    for sector_id, group in grouped.items():
        ...
        sorted_group = sorted(
            oriented_group,
            key=lambda item: (-float(item["_local_minor"]), float(item["_local_major"])),
        )
...
def format_sector_label(sector_id: int) -> str:
    """Format a sector ID for human-readable plant IDs."""
    if sector_id == -1:
        return "NOISE"
